...
module.exports = function (grunt) {
  ...
  grunt.registerMultiTask('convert_json', 'Convert a file to a json file', function () {
    ...
    this.files.forEach(function (f) {
      ...
      var lastKey = '', lastValue = '', lastNum = 0;

      grunt.log.writeln('Converting "' + f.src + '".');

      if (srcType === 'txt') { // convert to JSON
        data = {};
        data['id'] = f.src[0].replace(/\//g,'-').replace(/\./g, '_').replace(/txt$/, 'json').slice(4); // give each book an id, replacing values as needed
        srcFiles.split(/\r?\n/).forEach(function (line) {
          var lineParts = line.split(options.delimiter);

          //chapters
          if (lineParts[0].charCodeAt(2) === 67 && lineParts[1] === undefined) {
            //write the chapter
            if (lastKey !== '') {
                data[lastKey] = (options.onlyKeys) ? '' : lastValue.trim();
                lastValue = '';
            }

            lastKey = 'Chapter' + lastNum++; //chapter key
            // grunt.log.writeln('last key ' + lastKey);
          }

          if (lineParts[0].charCodeAt(2) !== 67 && lineParts[1] === undefined) {
            lastValue += ' ' + lineParts[0].trim(); //lines
            // grunt.log.writeln('last value ' + lastValue);
          }
        ...
        //write final chapter
        if (lastKey !== '') {
            data[lastKey.trim()] = (options.onlyKeys) ? '' : lastValue.trim();
        }
